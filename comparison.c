...
int audioChannel = 0; //change to get different channels in multichannel audio. 0 is default.
...
double AudioComparison(char* testFile, double** goal, int size){
	...
	double** test = NULL;
	int testsize = ReadAudioFile(testFile, &test);
	//printf("size: %d\n", testsize);

	double fitness;
	int i;
	if(testsize >= size){
		fitness = GetFitnessHelper(goal, test, size);
		for(i = size; i < testsize; i++){
